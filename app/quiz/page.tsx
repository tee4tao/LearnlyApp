...
function loadQuizStateFromLocalStorage(): QuizState {
  if (typeof window !== "undefined") {
    const stored = localStorage.getItem("quizState_v2");
    if (stored) {
      try {
...
function saveQuizStateToLocalStorage(state: QuizState) {
  if (typeof window !== "undefined") {
    localStorage.setItem("quizState_v2", JSON.stringify(state));
  }
}
...
const QuizPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-10">
      <h1 className="text-3xl font-bold mb-6">Two-Section Quiz</h1>
